...
/*
    This FSM scans the keyboard for a key press and key release by driving only 1 column to begin the scanning process.
...
module scanner_fsm (input logic clk, nrst,
    ...
    // detext row has gone high
    assign row_high = |(row_d);

    //Next State Logic
    always_comb begin
        case(current_state)
            col0_base:
                if (row_high)   next_state = col0_pressed;
                else            next_state = col1_base;
            col0_pressed:
                if (row_high)   next_state = current_state;
                else            next_state = col0_released;
            col0_released:      next_state = col0_base;
            col1_base:
                if (row_high)   next_state = col1_pressed;
                else            next_state = col2_base;
            col1_pressed:
                if (row_high)   next_state = current_state;
                else            next_state = col1_released;
            col1_released:      next_state = col1_base;
            col2_base:
                if (row_high)   next_state = col2_pressed;
                else            next_state = col3_base;
            col2_pressed:
                if (row_high)   next_state = current_state;
                else            next_state = col2_released;
            col2_released:      next_state = col2_base;
            col3_base:
                if (row_high)   next_state = col3_pressed;
                else            next_state = col0_base;
            col3_pressed:
                if (row_high)   next_state = current_state;
                else            next_state = col3_released;
            col3_released:      next_state = col3_base;
